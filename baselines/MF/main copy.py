...
def prepare_training_data(user, item_ratings):
...
def prepare_validate_data(user, item_ratings):
    nfeedback = len(item_ratings)
    all_items = [item for item, rating in item_ratings]
    ret = []
    if nfeedback >= 3:
...
def prepare_indomain_cold_start_test(user, item_ratings, user2negsamples):
    nfeedback = len(item_ratings)
    ret = []
    if nfeedback >= 3:
        target_item, target_rating = item_ratings[-1]
...
def data_partition(dataset_name):
    ...
    for line in tqdm(sequential_data, desc="building datasets", ncols=80):
        ...
        if len(from_domain_item_ratings):
            if user_type == 1:
                data_indomain_cold_start_test.extend(prepare_indomain_cold_start_test(user, from_domain_item_ratings, user2negsamples))
            elif user_type == 2:
                data_train.extend(prepare_training_data(user, from_domain_item_ratings))
                data_valid.extend(prepare_validate_data(user, from_domain_item_ratings))
    ...
    valid_dataset = MFTrainingDataset(data_valid, negpool)
    point_test_dataset = MFPointTestingDataset(data_test)
    list_test_dataset = MFListTestingDataset(data_test)
    indomain_point_test_dataset = MFPointTestingDataset(data_indomain_cold_start_test)
    indomain_list_test_dataset = MFListTestingDataset(data_indomain_cold_start_test)
