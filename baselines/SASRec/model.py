...
class SASRec(torch.nn.Module):
    def __init__(self, item_num, args):
        ...
        self.output_size = args.hidden_units * 2
        ...
        self.pos_emb = torch.nn.Embedding(args.max_his_len, self.output_size) # TO IMPROVE
        ...
        for _ in range(args.num_blocks):
            new_attn_layernorm = torch.nn.LayerNorm(self.output_size, eps=1e-8)
            self.attention_layernorms.append(new_attn_layernorm)

            new_attn_layer = torch.nn.MultiheadAttention(self.output_size,
                                                            args.num_heads,
                                                            args.dropout_rate)
            self.attention_layers.append(new_attn_layer)

            new_fwd_layernorm = torch.nn.LayerNorm(self.output_size, eps=1e-8)
            self.forward_layernorms.append(new_fwd_layernorm)

            new_fwd_layer = PointWiseFeedForward(self.output_size, args.dropout_rate)
            self.forward_layers.append(new_fwd_layer)

        self.trans_layer = torch.nn.Linear(args.hidden_units*2, args.hidden_units)
    ...
    def forward(self, batch):
        ...
        neg_items_embs = self.item_emb(neg_items)
        pos_masks = pos_items.ne(0)
        neg_masks = neg_items.ne(0)
        pos_logits = self.sigmoid((personalized_rep * pos_item_embs).sum(dim=-1))
        pos_loss = (torch.nn.MSELoss(reduction='none')(pos_logits, torch.ones_like(pos_logits, device=pos_logits.device)) * pos_masks).sum()
        neg_logits = self.sigmoid((personalized_rep * neg_items_embs).sum(dim=-1))
        neg_loss = (torch.nn.MSELoss(reduction='none')(neg_logits, torch.zeros_like(neg_logits, device=neg_logits.device)) * neg_masks).sum()
        return (pos_loss + neg_loss) / torch.max(torch.tensor(1, device=pos_masks.device), torch.sum(pos_masks)+torch.sum(neg_masks))
    ...
    def point_predict(self, batch):
        ...
        logits = self.sigmoid((personalized_rep * item_embs).sum(dim=-1))
        return logits
