...
parser.add_argument('--max_user_len', default=512, type=int)
...
def process_description(reviews):
    return review_tokenizer(
        reviews,
        padding="max_length",
        truncation=True,
        max_length=args.max_user_len,
        return_tensors="pt",
        verbose=False,
    )
...
class SASRecTrainingDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if "all_items" in data:
            data["target_item"] = sample(self.negpool, data["all_items"], 1)[0]
        batch = {
            "user_description": process_description(data["user_description"]),
    ...
    def collate_fn(self, batch):
        ...
        for key in batch[0]["user_description"]:
            batch_entry["user_description"][key] = torch.stack([data["user_description"][key].squeeze(0) for data in batch])
        return batch_entry
...
def build_his_data(user, item_ratings, item2meta, review_dict):
    ...
    for his_item, his_rating in item_ratings:
        ...
        if (user, his_item) in review_dict:
            review = review_dict[(user, his_item)]
        else:
            review = "No review"
...
def prepare_training_data(user, item_ratings, item2meta, review_dict):
    ...
    for idx in range(1, len_for_train):
        target_item, target_rating = item_ratings[idx]
        user_description = build_his_data(user, item_ratings[:len_for_train-1], item2meta, review_dict)
        ret.append({
...
def prepare_validate_data(user, item_ratings, negpool, item2meta, review_dict):
    nfeedback = len(item_ratings)
    ret = []
    if nfeedback >= 3:
        target_item, target_rating = item_ratings[-2]
        user_description = build_his_data(user, item_ratings[:-2], item2meta, review_dict)
...
def prepare_testing_data(user, item_ratings, user2negsamples, item2meta, review_dict):
    nfeedback = len(item_ratings)
    ret = []
    if nfeedback >= 3:
        target_item, target_rating = item_ratings[-1]
        user_description = build_his_data(user, item_ratings[:-1], item2meta, review_dict)
        data = {
            "user_description": user_description,
            "target_item": target_item,
            "target_rating": target_rating,
    ...
    for line in tqdm(sequential_data, desc="building datasets", ncols=80):
        ...
        if len(from_domain_item_ratings):
            if not check_test_complete(data_indomain_cold_start_test):
                data_indomain_cold_start_test.extend(prepare_testing_data(user, from_domain_item_ratings, user2negsamples, item2meta, review_dict))
            else:
                data_train.extend(prepare_training_data(user, from_domain_item_ratings, item2meta, review_dict))
                data_valid.extend(prepare_validate_data(user, from_domain_item_ratings, negpool, item2meta, review_dict))
                data_test.extend(prepare_testing_data(user, from_domain_item_ratings, user2negsamples, item2meta, review_dict))
    ...
    optimizer_grouped_parameters = [
        {
            "params": [
                ...
                if ("user_encoder" not in n and p.requires_grad)
            ],
            "lr": args.lr
        },
        {
            "params": [
                p for n, p in model.named_parameters()
                if ("user_encoder" in n and p.requires_grad)
            ],
            "lr": 1e-5
        }
    ]
